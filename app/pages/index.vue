...
watch(searchQuery, async (newQuery) => {
  ...
  const response = await fetch(`https://api.spotify.com/v1/search?q=name:$${encodeURIComponent(newQuery)}&type=track&limit=10`, {
    headers: {
...
.track-item { display: flex; align-items: center; cursor: pointer; margin-bottom: 8px; }
.track-item:hover { background-color: #f0f0f0; }
.track-item img { width: 50px; height: 50px; margin-right: 10px; }
</style>